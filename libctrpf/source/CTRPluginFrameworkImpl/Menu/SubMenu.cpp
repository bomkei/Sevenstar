...
void SubMenu::Draw(void)
{
  ...
  // Draw background
  Renderer::DrawRect2(background,
                      Preferences::Settings.BackgroundMainColor,
  ...
  for (u32 i = 0; i < _options.size(); i++) {
    ...
    if (i == _selector) {
      // If an action is selected, create a little effect (yeah, very
      // little)
...
void SubMenu::ProcessEvent(const Event &event)
{
  ...
  if (event.type == Event::EventType::KeyPressed) {
    switch (event.key.code) {
      ...
      case Key::DPadDown:
        _selector =
          std::min((u32)_selector + 1, (u32)_options.size() - 1);
        break;
      case Key::A:
        _action = true;
        _fadeClock.Restart();

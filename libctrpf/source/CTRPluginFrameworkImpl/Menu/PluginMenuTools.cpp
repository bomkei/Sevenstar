...
{
  const u8 tryOpenFilePat1[] = {0x0D, 0x10, 0xA0, 0xE1, 0x00, 0xC0,
                                0x90, 0xE5, 0x04, 0x00, 0xA0, 0xE1,
                                0x3C, 0xFF, 0x2F, 0xE1};
  const u8 tryOpenFilePat2[] = {0x10, 0x10, 0x8D, 0xE2, 0x00, 0xC0,
                                0x90, 0xE5, 0x05, 0x00, 0xA0, 0xE1,
                                0x3C, 0xFF, 0x2F, 0xE1};
...
static u32 FsTryOpenFileCallback(u32 a1, u16 *fileName, u32 mode)
{
  ...
  return HookContext::GetCurrent().OriginalFunction<u32>(a1, fileName,
                                                         mode);
}
...
static void _WriteLoadedFiles(MenuEntryTools *entry)
{
  ...
  if (entry->WasJustActivated()) {
    ...
    std::string filename = Utils::Format(
      "[%016llX] - LoadedFiles.txt", Process::GetTitleID());

    if (File::Open(g_hookExportFile, filename, mode) != 0) {
      OSD::Notify(std::string("Error: couldn't open \"")
                    .append(filename)
                    .append("\""),
...
{
  Keyboard kb(
    Color::LimeGreen << "Screenshot settings\x18\n\n Which screen(s) "
...
{
  // Update entry
  const char *screens[3] = {"Top screen", "Bottom screen",
                            "Both screens"};

  g_screenshotEntry->name =
    ("Screenshot: " << Color::LimeGreen)
    ...
    switch (choice) {
      ...
      case 1:  ///< Hotkeys
      {
        ...
        (HotkeysModifier(keys,
                         "Select the hotkeys you'd like to use to "
      ...
      case 2:  ///< Timer
      {
        ...
        keyboard.OnKeyboardEvent(
          [](Keyboard &kb, KeyboardEvent &event) {
...
static void EditBacklight(MenuEntryTools *entry)
{
  ...
  while (true) {
    ...
    message += "  - Bottom screen: " +
               std::to_string(Preferences::Backlights[1].value) +
               ", " +
    ...
    kb.Populate(
      {triggerTop, "Top screen", triggerBottom, "Bottom screen"});
    userchoice = kb.Open();
    if (userchoice == -1)
      return;
...
void PluginMenuTools::InitMenu(void)
{
  ...
  _mainMenu.Append(new MenuEntryTools(
    ...
    Icon::DrawRAM));
  _mainMenu.Append(new MenuEntryTools(
    "Screenshots", nullptr, Icon::DrawUnsplash, new u32(SCREENSHOT)));
  _mainMenu.Append(new MenuEntryTools("Miscellaneous", nullptr,
                                      Icon::DrawMore,
                                      new u32(MISCELLANEOUS)));
  _mainMenu.Append(new MenuEntryTools("Settings", nullptr,
                                      Icon::DrawSettings, this));
  _mainMenu.Append(new MenuEntryTools("Shutdown the 3DS", Shutdown,
                                      Icon::DrawShutdown));
  _mainMenu.Append(
  ...
  _screenshotMenu.Append(
    new MenuEntryTools("Change screenshot settings",
                       ScreenshotMenuCallback, Icon::DrawSettings));
  _screenshotMenu.Append(
    (g_screenshotEntry = new MenuEntryTools(
       "Screenshot: " << Color::LimeGreen
                      << KeysToString(Screenshot::Hotkeys) << "\x18, "
                      << Color::Orange << "Both screens",
       Screenshot_Enabler, true)));

  // Miscellaneous menu
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display loaded files", _DisplayLoadedFiles, true));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Write loaded files to file", _WriteLoadedFiles, true));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display touch cursor",
    [] {
      Preferences::Toggle(Preferences::DrawTouchCursor);
    },
    true, Preferences::IsEnabled(Preferences::DrawTouchCursor)));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display touch coordinates",
    [] {
      Preferences::Toggle(Preferences::DrawTouchPosition);
    },
    true, Preferences::IsEnabled(Preferences::DrawTouchPosition)));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display top screen's fps",
    [] {
      Preferences::Toggle(Preferences::ShowTopFps);
    },
    true, Preferences::IsEnabled(Preferences::ShowTopFps)));
  _miscellaneousMenu.Append(new MenuEntryTools(
    "Display bottom screen's fps",
    [] {
      Preferences::Toggle(Preferences::ShowBottomFps);
    },
    true, Preferences::IsEnabled(Preferences::ShowBottomFps)));

  // Settings menu
  _settingsMenu.Append(new MenuEntryTools("Change menu hotkeys",
                                          MenuHotkeyModifier,
                                          Icon::DrawGameController));

  _settingsMenu.Append(new MenuEntryTools(
    "Set backlight (Experimental)", EditBacklight, false, false));
  _settingsMenu.Append(new MenuEntryTools(
    "Use floating button",
    [] {
      Preferences::Toggle(Preferences::UseFloatingBtn);
    },
    true, Preferences::IsEnabled(Preferences::UseFloatingBtn)));

  _settingsMenu.Append(new MenuEntryTools(
    "Auto save enabled cheats",
    [] {
      Preferences::Toggle(Preferences::AutoSaveCheats);
    },
    true, Preferences::IsEnabled(Preferences::AutoSaveCheats)));
  _settingsMenu.Append(new MenuEntryTools(
    "Auto save favorites",
    [] {
      Preferences::Toggle(Preferences::AutoSaveFavorites);
    },
    true, Preferences::IsEnabled(Preferences::AutoSaveFavorites)));
  _settingsMenu.Append(new MenuEntryTools(
    "Auto load enabled cheats at starts",
    [] {
      Preferences::Toggle(Preferences::AutoLoadCheats);
    },
    true, Preferences::IsEnabled(Preferences::AutoLoadCheats)));
  _settingsMenu.Append(new MenuEntryTools(
    "Auto load favorites at starts",
    [] {
      Preferences::Toggle(Preferences::AutoLoadFavorites);
    },
    true, Preferences::IsEnabled(Preferences::AutoSaveFavorites)));
  _settingsMenu.Append(new MenuEntryTools(
...
void PluginMenuTools::_RenderTop(void)
{
  ...
  /*  // Window
    ...
    // Title
    int posY = 25;
    int xx = Renderer::DrawSysString("Tools", 40, posY, 330,
    Color::Blank); Renderer::DrawLine(40, posY, xx, Color::Blank);*/
...
void PluginMenuTools::_RenderBottom(void)
{
  ...
  // Draw Framework version
  {
    ...
    /*
    static const u32 xpos = (320 - Renderer::LinuxFontSize(version)) /
    2;

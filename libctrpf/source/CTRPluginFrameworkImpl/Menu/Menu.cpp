  ...
  : _noteTB("", "", IntRect(40, 30, 320, 180))
{
  drawFooter = false;
  _root = _folder =
    folder == nullptr ? new MenuFolderImpl("Menu") : folder;
  _iconCallback = iconCallback;
  _lastNoteItem = _lastSelectedItem = nullptr;
  _selector = 0;
  _reverseFlow = _selectedNameSize = 0;
...
MenuFolderImpl *Menu::Open(MenuFolderImpl *folder, int selector)
{
  ...
  _selector = std::max(
    (int)0, (int)std::min(selector, (int)(folder->ItemsCount() - 1)));
  _folder = folder;
...
{
  for (auto iter = _folder->_items.begin();
       iter < _folder->_items.end(); ++iter) {
    if (*iter == item) {
...
void Menu::Draw(void) const
{
  ...
  // Draw title
  int width = Renderer::DrawSysString(_folder->name.c_str(), posX,
                                      posY, XMAX, title);
  ...
  if (_iconCallback != nullptr) {
    for (; i < max; i++) {
      ...
      if (item->_type != MenuType::Folder) {
        ...
        Renderer::DrawSysString(item->name.c_str(), posX + 20, posY,
                                XMAX, c);
      }
      // Draw Folder
      else
        Renderer::DrawSysFolder(item->name.c_str(), posX, posY, XMAX,
                                c);
    ...
    for (; i < max; i++) {
      ...
      if (item->_type == MenuType::Entry) {
        Renderer::DrawSysCheckBox(
          item->name.c_str(), posX, posY, XMAX, c,
          item->AsMenuEntryImpl().IsActivated());
        // Renderer::DrawSysString(item->name.c_str(), posX + 20,
        // posY, XMAX, c);
      }
      // MenuEntryTools
        ...
        if (e->UseCheckBox) {
          Renderer::DrawSysCheckBox(item->name.c_str(), posX, posY,
                                    XMAX, c, e->IsActivated());
        }
        else {
          if (e->Icon != nullptr)
            e->Icon(posX, posY);
      ...
      // MenuEntryActionReplay
        ...
        Renderer::DrawSysCheckBox(
          item->name.c_str(), posX, posY, XMAX, c,
          item->AsMenuEntryImpl().IsActivated(), offset);
      }
      // MenuFolderImpl
      else
        Renderer::DrawSysFolder(item->name.c_str(), posX, posY, XMAX,
  ...
  if (drawFooter) {
    ...
    if (!_folder->GetNote().empty()) {
      ...
      Renderer::DrawSysStringReturn(
        (u8 *)_folder->GetNote().c_str(), 35, posY, 295,
        Preferences::Settings.MainTextColor, 190);
    }
  }
}
    ...
    switch (event.key.code) {
      case A: {
        ...
        // MenuEntryTools
          ...
          MenuEntryTools *e =
            reinterpret_cast<MenuEntryTools *>(item);
        ...
        /*else if (item->_type == MenuType::FreeCheat)
        {
            MenuEntryFreeCheat *e =
        reinterpret_cast<MenuEntryFreeCheat *>(item);
        ...
        MenuFolderImpl *folder =
          reinterpret_cast<MenuFolderImpl *>(item);
...
void Menu::_ScrollUp(int step)
{
  ...
  if (step > 0)
    _selector =
      std::max(0, static_cast<int>(_folder->ItemsCount() - 1));
  _input.Restart();
}
...
void Menu::_ScrollDown(int step)
{
  if (!_input.HasTimePassed(Milliseconds(200)))
    return;
  while (_selector < static_cast<int>(_folder->ItemsCount() - 1) &&
         step-- > 0)
    _selector++;
  if (step > 0)
    _selector = 0;

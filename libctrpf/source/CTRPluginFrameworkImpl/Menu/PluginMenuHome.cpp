...
static const char g_copyrightText[] =
...
PluginMenuHome::PluginMenuHome(std::string &name, bool showNoteBottom)
  :
    ...
    _noteTB("", "",
            showNoteBottom ? IntRect(20, 46, 280, 124)
                           : IntRect(40, 30, 320, 180)),

    _showStarredBtn(
      Button::Toggle | Button::Sysfont | Button::Rounded, "Favorite",
      IntRect(30, 70, 120, 30), Icon::DrawFavorite),
    ...
    _toolsBtn(Button::Sysfont | Button::Rounded, "Tools",
              showNoteBottom ? IntRect(99, 172, 120, 30)
                             : IntRect(165, 140, 120, 30),
              Icon::DrawTools),

    // _closeBtn(*this, nullptr, IntRect(275, 24, 20, 20),
    // Icon::DrawClose),
    _keyboardBtn(Button::Icon, IntRect(130, 30, 25, 25),
                 Icon::DrawKeyboard),
    _controllerBtn(Button::Icon, IntRect(170, 30, 25, 25),
                   Icon::DrawGameController25),

    _AddFavoriteBtn(Button::Icon | Button::Toggle,
                    IntRect(50, 30, 25, 25), Icon::DrawAddFavorite),
             ...
             Icon::DrawInfo)
{
  ...
  g_textXpos[1] =
    (320 - Renderer::LinuxFontSize(g_copyrightText)) / 2;
...
bool PluginMenuHome::operator()(EventList &eventList, int &mode,
                                Time &delta)
{
  static Task top(
    [](void *arg) {
...
static void ScrollDown(int &selector, MenuFolderImpl &folder,
                       int step)
{
  // If there's no selectable entry in the folder, return
  if (!SelectableEntryCount(folder))
...
void PluginMenuHome::_RenderTop(void)
{
  ...
  const Color &unselected =
    Preferences::Settings.MenuUnselectedItemColor;
  ...
  int posYbak = posY;
  int width = Renderer::DrawSysString(folder->name.c_str(), posX,
                                      posY, maxWidth, maintext);
  Renderer::DrawLine(posX, posY, width, maintext);
  posY += 7;

  if (_showVersion && !_starMode && !folder->HasParent())
    Renderer::DrawSysString(_versionStr.c_str(), _versionPosX,
                            posYbak, 360, maintext);
  ...
  for (; i < max; i++) {
    ...
    if (flags.useSeparatorBefore) {
      if (flags.useStippledLineForBefore)
        Renderer::DrawStippledLine(posX, posY - 1, 320, unselected,
                                   1);
      else
    ...
    if (item->_type == MenuType::Entry) {
      ...
      if (entry->GameFunc != nullptr)
        ...
        Renderer::DrawSysString(name, posX + 20, posY, 350, fg,
                                offset);
        posY += 1;
      }
    }
...
void PluginMenuHome::_showStarredBtn_OnClick(void)
{
  ...
  if (f->ItemsCount() == 0) {
    ...
    MenuEntryImpl *e =
      reinterpret_cast<MenuEntryImpl *>(f->_items[_selector]);
...
void PluginMenuHome::_keyboardBtn_OnClick(void)
{
  MenuFolderImpl *f = _starMode ? _starred : _folder;
  MenuEntryImpl *e =
    reinterpret_cast<MenuEntryImpl *>((*f)[_selector]);
  ...
  if (item != nullptr) {
    ...
    if (count == 1) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (it == item) {
          _starredConst->_items.erase(_starredConst->_items.begin() +
                                      i);
          break;
        }
      }
...
void PluginMenuHome::Init(void)
{
  MenuFolderImpl *folder = _starMode ? _starred : _folder;
  MenuItem *item =
    folder->ItemsCount() != 0 ? folder->_items[0] : nullptr;

...
{
  // If the callback is going to be added, make sure it's not in the
  // trash bin
  {
    Lock lock(_trashBinMutex);
    if (_callbacksTrashBin.size()) {
      auto it = std::find(_callbacksTrashBin.begin(),
                          _callbacksTrashBin.end(), callback);
...
int PluginMenuImpl::Run(void)
{
  ...
  EventManager openManager(
    EventManager::EventGroups::GROUP_KEYS |
  ...
  while (_pluginRun) {
    ...
    while ((_isOpen && openManager.PollEvent(event)) ||
           (!_isOpen && closedManager.PollEvent(event)) ||
           _forceOpen) {
    ...
    else  // menu is closed
    {
      ...
      // static KeySequenceImpl konamicode({ DPadUp, DPadUp, DPadDown,
      // DPadDown, DPadLeft, DPadRight, DPadLeft, DPadRight, B, A, B,
      // A });
  ...
  const Preferences::Header &header, File &settings)
{
  ...
  if (settings.Read(uids.data(),
                    sizeof(u32) * header.enabledCheatsCount) == 0) {
    for (u32 &uid : uids) {
...
void PluginMenuImpl::LoadFavoritesFromFile(
  const Preferences::Header &header, File &settings)
{
  if (_runningInstance == nullptr)
    return;
  ...
  if (settings.Read(uids.data(),
                    sizeof(u32) * header.favoritesCount) == 0) {
    for (u32 &uid : uids) {
      MenuItem *item = folder->GetItem(uid);
...
void PluginMenuImpl::LoadHotkeysFromFile(
  const Preferences::Header &header, File &settings)
{
  ...
  for (size_t count = 0; count < header.hotkeysCount; count++) {
    if (settings.Read(buffer, sizeof(u32) * 2) == 0) {
      if (settings.Read(&buffer[2], sizeof(u32) * buffer[1]) == 0) {
        ...
        HotkeyManager::OnHotkeyChangeClbk callback =
          entry->Hotkeys._callback;
...
void PluginMenuImpl::WriteEnabledCheatsToFile(
  Preferences::Header &header, File &settings)
{
  if (_runningInstance == nullptr)
    return;
                                    ...
                                    MenuFolderImpl *folder, u32 &size)
{
  ...
  for (MenuItem *item : folder->_items) {
    ...
    if (item->IsFolder()) {
      ExtractHotkeys(hotkeys,
                     reinterpret_cast<MenuFolderImpl *>(item), size);
      continue;
    }

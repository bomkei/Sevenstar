...
void KeyboardImpl::ChangeSelectedEntry(int entry)
{
  if (!_customKeyboard)
    return;
  if (!_strKeys.size() ||
      entry >= static_cast<int>(_strKeys.size()) ||
    ...
    if (ico.sizeX != 30 || ico.sizeY != 30)
      tks = new TouchKeyString(
        CustomIcon(Icon::DefaultCustomIcon.pixArray,
                   Icon::DefaultCustomIcon.sizeX,
                   Icon::DefaultCustomIcon.sizeY, ico.isEnabled),
        box, true);
    else
...
int KeyboardImpl::Run(void)
{
  ...
  if (!_customKeyboard) {
    ...
    if ((!_keys->at(15).IsEnabled() || !_keys->at(16).IsEnabled()) &&
        _mustRelease &&
        (_layout == DECIMAL || _layout == HEXADECIMAL)) {
...
void KeyboardImpl::_RenderTop(void)
{
  ...
  Renderer::DrawSysStringReturn(
    reinterpret_cast<const u8 *>(_text.c_str()), posX, posY, maxX,
  ...
  if (_errorMessage && !_error.empty()) {
    if (posY < 120)
      posY += 48;
    Renderer::DrawSysStringReturn(
      reinterpret_cast<const u8 *>(_error.c_str()), posX, posY, maxX,
      red, maxY);
  }
}
...
void KeyboardImpl::_RenderBottom(void)
{
  ...
  if (!_customKeyboard) {
    ...
    // Draw input
    Renderer::DrawSysString(_userInput.c_str(), posX, posY, 300,
                            theme.Input, _offset);

    // Draw cursor
    if (_showCursor &&
        _blinkingClock.GetElapsedTime() < Seconds(0.5f))
      Renderer::DrawLine(_cursorPositionOnScreen + posX, 21, 1,
                         theme.Cursor, 16);
  ...
  // Draw custom keyboard
    ...
    for (size_t i = _currentPosition; i < max && i < _strKeys.size();
         i++) {
    ...
    Renderer::DrawLine(posX, posY + 1, 1, sbBackground,
                       _scrollbarSize - 2);
    Renderer::DrawLine(posX + 1, posY, 1, sbBackground,
                       _scrollbarSize);
    Renderer::DrawLine(posX + 2, posY + 1, 1, sbBackground,
                       _scrollbarSize - 2);

    posY += (int)(_scrollPosition);

    // Draw thumb
    Renderer::DrawLine(posX, posY + 1, 1, sbThumb,
                       _scrollCursorSize - 2);
    Renderer::DrawLine(posX + 1, posY, 1, sbThumb, _scrollCursorSize);
    Renderer::DrawLine(posX + 2, posY + 1, 1, sbThumb,
                       _scrollCursorSize - 2);
  }
}
...
void KeyboardImpl::_ProcessEvent(Event &event)
{
  ...
  if (event.type == Event::KeyPressed) {
    ...
    if (!_customKeyboard && event.key.code == Y &&
        !_userInput.empty()) {
      keyPressIntended = true;
      _userInput.clear();
      _ClearKeyboardEvent();
    ...
    if (event.key.code == X && !_customKeyboard &&
        _layout != QWERTY && _canChangeLayout) {
      keyPressIntended = true;
      _userInput.clear();
      _ClearKeyboardEvent();
      _KeyboardEvent.type = KeyboardEvent::EventType::InputWasCleared;
  ...
  if (!keyPressIntended && _onKeyboardEvent != nullptr &&
      _owner != nullptr &&
...
void KeyboardImpl::_UpdateScroll(float delta, bool ignoreTouch)
{
  ...
  if (_displayScrollbar) {
    if (!_manualScrollUpdate)
      _scrollSize =
        (_inertialVelocity * INERTIA_SCROLL_FACTOR * delta);
...
void KeyboardImpl::_ScrollUp(void)
{
...

  {
    ...
    const int units =
      UnitsToNextChar(cstr + _cursorPositionInString,
                      strLength - _cursorPositionInString);

    if (units < 0) {
      ///< Weird char being found, better clear the input than
      ///< abort/crash later
      goto error;
    }
...
void KeyboardImpl::_ScrollDown(void)
{
  ...
  // If cursor is beyond the string, fix it (can happen when deleting
  // a unicode)
  if (_cursorPositionInString > static_cast<int>(strLength))
    _cursorPositionInString = strLength;

  {
    ...
    const int units = UnitsToPreviousChar(
      cstr + _cursorPositionInString, _cursorPositionInString);
...
void KeyboardImpl::_UpdateScrollInfos(void)
{
  ...
  const int unitsToCursor =
    UnitsToAdvance(cstr, _cursorPositionInString);
    ...
    if (ret != -1) {
      ...
      if (ret == 0x12345678) {
        if ((_layout == DECIMAL && _userInput.size() >= 18) ||
            (_layout == QWERTY && _max &&
             Utils::GetSize(_userInput) >= _max))
          return (false);

        _ClearKeyboardEvent();
        _KeyboardEvent.type = KeyboardEvent::CharacterAdded;
        decode_utf8(&_KeyboardEvent.codepoint,
                    (const u8 *)temp.c_str());
...
bool KeyboardImpl::_CheckInput(void)
{
  ...
  // In case there's no convert function, always consider input as
  // valid
  if (_convert == nullptr)
    return (true);
  ...
  if (_isIconKeyboard) {
    if (_manualKey == -1) {
      if (key & (Key::Down | Key::Left)) {
        ...
        int tempKey = _displayScrollbar
                        ? std::min((int)_strKeys.size() - 1,
                                   _currentPosition + 20)
                        : _strKeys.size() - 1;
        if (!_strKeys[tempKey]->CanUse())
    ...
    if (key & (Key::Down | Key::Up | Key::Left | Key::Right)) {
      ...
      if (_displayScrollbar) {
        ...
        if (keyRow > positionRow + 6 - 2) {
          positionRow = keyRow - 4;
          _scrollSize = ((positionRow * 36.01f) + 15) / _scrollJump -
                        _scrollPosition;
          _manualScrollUpdate = true;
          _UpdateScroll(0.f, true);
        }
    ...
    if (_manualKey == -1) {
      if (key & Key::Down) {
        ...
        int tempKey =
          _displayScrollbar
            ? std::min((int)_strKeys.size() - 1, _currentPosition + 5)
            : _strKeys.size() - 1;
        if (!_strKeys[tempKey]->CanUse())
          tempKey = (int)_strKeys.size() - 1;
...
// WIll only be used in the hex editor, so no need to do a full
// implementation

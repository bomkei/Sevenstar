...
HotkeysModifier::HotkeysModifier(u32 &keys,
    ...
    _message(message)
{
  ...
  for (int i = 0, posY = 32; i < 7; ++i, posY += 25) {
    Button b(Button::Icon | Button::Toggle,
             IntRect(200, posY, 20, 20), Icon::DrawCheckBox);
    _checkboxs.push_back(b);
  }
  for (int i = 0; i < 16; ++i) {
    if (keys & (1u << i)) {
...
void HotkeysModifier::operator()(void)
{
  ...
  u32 oldKeys = _keys;
  while (
    ((!Window::BottomWindow.MustClose() && !mustclose) || !_keys) &&
    !sleepClose) {
    Controller::Update();
...
void HotkeysModifier::_DrawTop(void) const
{
  ...
  int posY = 61;
  Renderer::DrawSysStringReturn((const u8 *)_message.c_str(), 40,
                                posY, 300,
...
void HotkeysModifier::_DrawBottom(void)
{
  ...
  int skip =
    (!System::IsNew3DS() || !settings.AreN3DSButtonsAvailable) ? 1
                                                               : 0;

  // Draw labels
  for (int i = 0 + skip, posY = 32 + (skip * 25); i < 7;
       ++i, posY += 9)
    Renderer::DrawSysString(stable[i], 50, posY, 290,
                            Preferences::Settings.MainTextColor);
  for (int i = 7 + skip, posY = 32 + (skip * 25); i < 14;
       ++i, posY += 9)

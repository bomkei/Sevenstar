    ...
    _cursor(0)
{
  ...
  Renderer::GetTextInfos(title.c_str(), lineCount, maxTitleWidth,
                         320.f);
  Renderer::GetTextInfos(message.c_str(), lineCount, maxLineWidth,
                         320.f);
...
void MessageBoxImpl::_Draw(void)
{
  ...
  // int maxW = _box.leftTop.x + _box.size.x - 5; ///< Should be -10,
  // but that way a letter that is slighty too big can still be drawn
  // int maxH = _box.leftTop.y + _box.size.y - 30;

  /*if (!_title.empty())
  {
      int width = Renderer::DrawSysString(_title.c_str(), posX, posY,
  maxW, _titleColor); Renderer::DrawLine(posX, posY, width - posX +
  30, _titleColor); posY += 8;
  }
  Renderer::DrawSysStringReturn((const u8 *)_message.c_str(), posX,
  posY, maxW, settings.MainTextColor, maxH);
  */
  _textbox.Draw();
  // Draw "Buttons"
  // posY += 13;

...
FloatingButton OSDImpl::FloatingBtn(IntRect(0, 0, 40, 40),
                                    Icon::DrawRocket);
...
void OSDImpl::_Initialize(void)
{
  ...
  for (u32 i = 0; i < sizeof(titlesNeedWritePrevFB) / sizeof(u64);
       i++) {
...
OSDImpl::OSDMessage::OSDMessage(const std::string &str,
...
void OSDImpl::Update(void)
{
  if (Preferences::IsEnabled(Preferences::UseFloatingBtn)) {
    FloatingBtn.Update(Touch::IsDown(),
...
bool OSDImpl::Draw(void)
{
  ...
  for (OSDMessage *message : Notifications) {
    ...
    Renderer::DrawString(message->text.c_str(), posX, posY,
                         message->foreground, message->background);
    posY += 5;
    if (!message->drawn)
...
u32 OSDImpl::MainCallback(u32 isBottom, int nextBank, void *leftFb,
                          void *rightFb, int stride, int format,
                          int swap)
{
...
Result OSDImpl::OnBottomScreenFrame(void)
{
  ...
  if (Touch::IsDown()) {
    ...
    if (Preferences::IsEnabled(Preferences::DrawTouchPosition)) {
      std::string &&str = Utils::Format("Touch.x: %d  Touch.y: %d",
                                        touchPos.x, touchPos.y);
      int posY = 20;
      Renderer::DrawString(str.c_str(), 10, posY, Color::White,
                           Color::Black);
    }
  }
...
void OSDImpl::CallbackCommon(u32 isBottom, void *addr, void *addrB,
                             int stride, int format, int swap)
{
  ...
  if (WritesToPrevFB) {
    // TO DO: This is a very hacky fix! Research why this actually
    // happens From my research, looks like puyo puyo games either
    // re-render or do some post-processing to the FB after CTRPF
    // writes to them, which causes the changes to be overwritten. As
    // a hacky solution, we actually draw to the FB that's currently
  ...
  if (!isBottom) {
    ScreenImpl::Top->Acquire((u32)addr, (u32)addrB, stride,
                             format & 0b111);
    Renderer::SetTarget(TOP);
    ...
    ScreenImpl::Bottom->Acquire((u32)addr, (u32)addrB, stride,
                                format & 0b111);
    ...
    Callbacks.erase(
      std::remove_if(Callbacks.begin(), Callbacks.end(),
                     [](OSDCallback cb) {
                       ...
                       auto foundIter = std::find(trashbin.begin(),
                                                  trashbin.end(), cb);
  ...
  // We need to ensure that notifications are handled before running
  // callbacks
  DrawNotifTask1.Wait();
  DrawNotifTask2.Wait();
  ...
  if (Callbacks.size()) {
    ...
    screen.Is3DEnabled =
      isBottom ? false : ScreenImpl::Top->Is3DEnabled();
...
void OSDImpl::UpdateScreens(void)
{
  ...
  TopScreen.RightFramebuffer =
    (u32)screen->GetRightFrameBuffer(false);
  ...
  BottomScreen.LeftFramebuffer =
    (u32)screen->GetLeftFrameBuffer(false);
  BottomScreen.RightFramebuffer =
    (u32)screen->GetRightFrameBuffer(false);
...
static const u32 g_OSDPattern[] = {
  ...
  0xEE078F9A,  // MCR p15, 0, R8, c7, c10, 4 // Data Synchronization
               // Barrier
  0xE3A03001,  // MOV R3, #1
  0xE7902104,  // LDR R2, [R0, R4, LSL#2] // 0x10

  0xEE076F9A,  // MCR p15, 0, R6, c7, c10, 4 // Data Synchronization
               // Barrier
  0xE3A02001,  // MOV R2, #1
...
static u32 SearchOSD(u32 pattern)
{
  ...
  if (pattern == 0) {
    address = memsearch((u8 *)0x100000, g_OSDPattern,
                        Process::GetTextSize(), 0x14);
    ...
    address = memsearch((u8 *)0x100000, &g_OSDPattern[9],
                        Process::GetTextSize(), 0x14);
  }
  else if (pattern == 4) {
...
void InstallOSD(void)
{
  ...
  if (!found) {
    ...
    if (!found) {
      ...
      OSDImpl::OSDHook
        .InitializeForMitm(found2, u32(OSDImpl::MainCallback2))
        .Enable();
      return;
    }
  }
  ...
  OSDImpl::OSDHook
    .InitializeForMitm(found, u32(OSDImpl::MainCallback))
    .Enable();
}

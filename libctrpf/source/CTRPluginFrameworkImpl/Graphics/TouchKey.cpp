...
TouchKey::TouchKey(int character, IntRect ui, bool isEnabled)
{
  ...
  _posX = ((ui.size.x - static_cast<int>(_glyph->Width())) >> 1) +
          ui.leftTop.x;
                   ...
                   bool isEnabled)
{
  ...
  _posX = ((ui.size.x - static_cast<int>(_content->width)) >> 1) +
          ui.leftTop.x;
  _posY = ((ui.size.y - 16) >> 1) + ui.leftTop.y;
}

TouchKey::TouchKey(int value, IconCallback icon, IntRect ui,
                   bool isEnabled)
{
  _character = value;
  _content = nullptr;
  ...
  // Icon drawing infos
  _posX =
    ((_uiProperties.size.x - 15) >> 1) + _uiProperties.leftTop.x;
  _posY =
    ((_uiProperties.size.y - 15) >> 1) + _uiProperties.leftTop.y;
}
...
void TouchKey::Draw(void)
{
  ...
  const Color &text =
    _isPressed ? theme.KeyTextPressed : theme.KeyText;
...
void TouchKey::Update(const bool isTouchDown,
                      const IntVector &touchPos)
{
  if (!_enabled)
    return;

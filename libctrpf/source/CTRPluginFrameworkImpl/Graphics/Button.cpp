...
Button::Button(u32 type, const std::string &content,
               const IntRect &bounds, IconCallback icon)
  : _icon{(void *)icon},
    _content{content},
    _bounds{bounds}
{
...
Button::Button(u32 type, const std::string &content,
               const IntRect &bounds, IconCallbackState icon)
...
// Shitty but avoid casts in code
Button::Button(u32 type, const IntRect &bounds,
...
void Button::Draw(void)
{
  ...
  const Color &fillColor =
    _isLocked
      ...
      : (_isPressed ? Color::DimGrey
                    : (_isToggleBtn && _state ? Color::LimeGreen
                                              : Color::Gainsboro));
  const Color &textColor = Color::Black;
  // const Color &borderColor = _isLocked ? Color::Gray :
  // Color::DarkGrey;

  // Background
  if (_isRounded)
    Renderer::DrawRoundedRectangle(_bounds, Color::DarkGrey,
                                   fillColor);
  else {
  ...
  if (_useSysfont)
    Renderer::DrawSysString(_content.c_str(), posX, posY, _limit,
                            textColor);
  else
    Renderer::DrawString((char *)_content.c_str(), posX, posY,
                         textColor);
}

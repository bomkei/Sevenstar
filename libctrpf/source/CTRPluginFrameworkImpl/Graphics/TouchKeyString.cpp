...
void TouchKeyString::Draw(void)
{
  ...
  const Color &background = (_isPressed || _isForcePressed)
                              ...
                              : theme.KeyBackground;
  const Color &text = (_isPressed || _isForcePressed)
                        ? theme.KeyTextPressed
                        : theme.KeyText;
  ...
  if (!_isIcon) {
    int posX = _posX;
    int posY =
      ((_uiProperties.size.y - 16) >> 1) + _uiProperties.leftTop.y;
...
void TouchKeyString::Update(const bool isTouchDown,

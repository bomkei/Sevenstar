...
{
  Result res =
    svcMapMemoryBlock(handle, (u32)sharedMem, myPerm, otherPerm);
  if (R_SUCCEEDED(res) && sharedMem &&
    ...
    u64 firstSysTickTS =
      reinterpret_cast<vu64 *>(sharedMemTouchScreen)[0];
    u32 arrayIndexTS =
      reinterpret_cast<vu32 *>(sharedMemTouchScreen)[4];

    u64 currSysTick = svcGetSystemTick();

    // VERY HACKY VERIFICATION THIS IS HID SHARED MEM!!!
    // Probably breaks if hid thread is paused just here, but the
    // likelyness of this happening is very low, so I guess it's good
    // enough...
    if (llabs(currSysTick - firstSysTick) < 1000000000ULL &&
        llabs(currSysTick - firstSysTickTS) < 1000000000ULL &&
...
void KeepThreadMain(void *arg UNUSED)
{
  ...
  SoundEngineImpl::SetVaToPaConvFunction([](const void *addr) {
  ...
  // Install CRO hook
  {
    ...
    u32 loadCroAddress = Utils::Search<u32>(
      0x00100000, Process::GetTextSize(), LoadCroPattern);
  ...
  // Install svcMapMemoryBlock hook
  {
    ...
    do {
      svcMapMemoryBlockAddr =
        Utils::Search<u32>(svcMapMemoryBlockAddr + 4,
                           Process::GetTextSize(), {0xEF00001F});
  ...
  settings.ThreadPriority =
    std::min(settings.ThreadPriority, (u32)0x3E);
  ...
  // Reduce priority
  while (R_FAILED(svcSetThreadPriority(g_keepThreadHandle,
                                       settings.ThreadPriority - 1)))
    ;
  ...
  while (true) {
        ...
        0x09401BFE)  // 0.1s
    {
      ...
      if (event == PLG_SLEEP_ENTRY) {
        SoundEngineImpl::NotifyAptEvent(
          APT_HookType::APTHOOK_ONSLEEP);
        ...
        SoundEngineImpl::NotifyAptEvent(
          APT_HookType::APTHOOK_ONSUSPEND);

        // Close csnd as it may be needed by other processes (4
        // sessions max.)
        ncsndExit();
        ...
        svcUnmapProcessMemoryEx(CUR_PROCESS_HANDLE, 0x01E80000,
                                0x2000);
        ...
        // Re-map hook memory
        svcMapProcessMemoryEx(CUR_PROCESS_HANDLE, 0x1E80000,
                              CUR_PROCESS_HANDLE,
        ...
        SoundEngineImpl::NotifyAptEvent(
          APT_HookType::APTHOOK_ONRESTORE);
...
void Initialize(void)
{
...

  {
    ...
    Screenshot::Prefix += Utils::Format(" - %08X] - Screenshot",
                                        (u32)Process::GetTitleID());
...
void ThreadExit(void)
{
  ...
  if (reinterpret_cast<u32>(
        ((Thread_tag *)threadGetCurrent())->stacktop) < 0x07000000) {
    // ## Main Thread ##
    ...
    ProcessImpl::IsPaused =
      std::min((u32)ProcessImpl::IsPaused, (u32)1);
  ...
  else  // We aborted in a very early stage, so just release the game
        // and exit
    svcSignalEvent(g_continueGameEvent);
  svcExitThread();
}
...
extern "C" int __entrypoint(int arg)
{
  ...
  // Wait until basic initialization has been made before returning to
  // game

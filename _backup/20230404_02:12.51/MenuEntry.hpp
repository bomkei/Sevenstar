...
public:
  explicit MenuEntry(const std::string &name,
                     const std::string &note = "");
  MenuEntry(const std::string &name, FuncPointer gameFunc,
            const std::string &note = "");
  MenuEntry(const std::string &name, FuncPointer gameFunc,
            FuncPointer menuFunc, const std::string &note = "");
  MenuEntry(int radioGroup, const std::string &name,
            FuncPointer gameFunc, const std::string &note = "");
  MenuEntry(int radioGroup, const std::string &name,
  ...
  /**
   * \brief Enable the entry (the gamefunction linked to it will be
   * executed) \param
   */
  ...
  MenuEntry *GetEnabled();

  bool AssignEnabledFlag(bool &flag);

  /**
   * \brief Hide the entry from the menu. The entry will also be
   * disabled
   */
  ...
  /**
   * \brief Set the entry as a radio entry and link it to a radio
   * group ID \param id The id of the radio group to be linked to
   */
   ...
   * \return true if the entry is visible, false if the entry is
   * hidden
   */
  bool IsVisible(void) const;

  /**
   * \brief Set if this entry must display a separator on top of the
   * entry \param type Type of separator to display
   */
  void UseTopSeparator(Separator type = Separator::Filled) const;

  /**
   * \brief Set if this entry must display a separator at the bottom
   * of the entry \param type Type of separator to display
   */
  void UseBottomSeparator(Separator type = Separator::Filled) const;

  /**
   * \brief Set if the entry can be selected in the menu or not.\n
   * If the entry is Activated and the state is set to unselectable,
   * the entry will be disabled \param canBeSelected
   */
  ...
  /**
   * \brief The hotkey manager of this entry. See MenuEntryHotkeys.hpp
   * for more infos
   */

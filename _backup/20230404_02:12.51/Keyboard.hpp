...
public:
  ...
  u32 codepoint{0};  ///< The codepoint of the character that thrown
                     ///< the event (used for CharacterAdded and
                     ///< CharacterRemoved, 0 otherwise)
  u32 selectedIndex{
    0};  ///< The entry index in a custom keyboard being selected
         ///< (used for SelectionChanged, 0 otherwise)
  Key affectedKey{
    (Key)0};  //< Button affected not mapped to any keyboard feature
              //(used for ButtonPressed, ButtonHold and
              //ButtonReleased, 0 otherwise)
};
...
class Keyboard {
   ...
   * \param input     A void pointer to the input, must be casted to
   * the correct type (as passed to Keyboard::Open) \param error     A
   * reference to the string that hold the error message. You can edit
   * it according to what you want to display \return A boolean \n
   * Return true if the input is valid, false if it's not
   */
  using CompareCallback = bool (*)(const void *, std::string &);

  /**
   * \brief The signature of the callback called when the input change
   * (user enter / delete a character) \param keyboard  A reference to
   * the Keyboard object that called the callback \event event     The
   * event that caused the input to change
   */
...
public:
   ...
   * \param text  The message to display on the top screen if
   * displayed
   */
   ...
   * \param canAbort  Whether the user can press B to close the
   * keyboard and abort the current operation
   */
   ...
   * \param maxValue The maximum count of characters that the user can
   * type
   */
  ...
  /**
   * \brief Define a callback that will be called when the user change
   * the input \n Note that if a CompareCallback is set,
   * CompareCallback is called before OnKeyboardEvent \n See
   * OnEventCallback's description for more infos \param callback
   */
  ...
  /**
   * \brief Selects and scroll to the specified entry, must be called
   * after Populate \param entry Entry index to select, -1 to deselect
   * all.
   */
  void ChangeSelectedEntry(int entry);

  /**
   * @brief Gets the last selected entry before the keyboard was
   * closed
   *
  ...
  /**
   * \brief Populate a keyboard with the strings contained in an
   * std::vector \param input  A std::vector that contain a list of
   * strings \param resetScroll  Set to true to reset the scroll
   * position if the list is the same size
   */
  void Populate(const std::vector<std::string> &input,
                bool resetScroll = false);

  /**
   * \brief Populate a keyboard with the CustomIcons contained in an
   * std::vector \param input A std::vector that contains a list of
   * CustomIcons (icon size must be 30x30 pixels, otherwise a red
   * cross will be displayed) \param resetScroll  Set to true to reset
   * the scroll position if the list is the same size
   */
  void Populate(const std::vector<CustomIcon> &input,
                bool resetScroll = false);
  ...
  /**
   * \brief Forcefully close the keyboard without any regard to the
   * error flag \n (This can only be called from an OnKeyboardEvent
   * callback)
   */
  ...
  /**
   * \brief This property define if the top screen must be displayed
   * or not \n Note that when disabled, errors messages can't be
   * displayed
   */

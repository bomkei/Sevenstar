...
ScreenImpl::ScreenImpl(const u32 lcdSetupInfo,
                       const u32 fillColorAddress,
...
{
  auto *screensFbs = reinterpret_cast<ScreensFramebuffers *>(
    FwkSettings::Header->heapVA);

  Top = new (_topBuf) ScreenImpl(
    0x10400400 | (1u << 31), (0x10202000 | (1u << 31)) + 0x204, true);
  Bottom = new (_botBuf) ScreenImpl(
    0x10400500 | (1u << 31), (0x10202000 | (1u << 31)) + 0xA04);
  ...
  fb->framebuf0_vaddr =
    reinterpret_cast<u32 *>(screensFbs->topFramebuffer0);
  ...
  fb->framebuf0_vaddr =
    reinterpret_cast<u32 *>(screensFbs->topFramebuffer1);
  fb->framebuf1_vaddr = fb->framebuf0_vaddr;
  fb->framebuf_widthbytesize = 240 * 2;  // Enforce RGB565
  fb->format = 0x42;  // GSP_RGB565_OES;
  ...
  fb->active_framebuf = 0;
  fb->framebuf0_vaddr =
    reinterpret_cast<u32 *>(screensFbs->bottomFramebuffer0);
  ...
  fb->active_framebuf = 1;
  fb->framebuf0_vaddr =
    reinterpret_cast<u32 *>(screensFbs->bottomFramebuffer1);
  fb->framebuf1_vaddr = nullptr;
  fb->framebuf_widthbytesize = 240 * 2;  // Enforce RGB565
  fb->format = GSP_RGB565_OES;
  fb->framebuf_dispselect = 1;
...
u32 ScreenImpl::Acquire(bool fade)
{
  ...
  _leftFrameBuffers[0] =
    reinterpret_cast<u32>(fbInfo[0].framebuf0_vaddr);
  _leftFrameBuffers[1] =
    reinterpret_cast<u32>(fbInfo[1].framebuf0_vaddr);
  _rightFrameBuffers[0] =
    reinterpret_cast<u32>(fbInfo[0].framebuf1_vaddr);
  _rightFrameBuffers[1] =
    reinterpret_cast<u32>(fbInfo[1].framebuf1_vaddr);
...
void ScreenImpl::Clear(bool applyFlagForCurrent)
{
  ...
  val |= (_isTopScreen ? GSP::FB_TOP_NEED_CLEAR
                       : GSP::FB_BOTTOM_NEED_CLEAR);
...
void ScreenImpl::Debug(void)
{
  ...
  _leftFramebuffers[0]).c_str(), 10, posY, Color::Blank,
  Color::Black); Renderer::DrawString(Utils::Format("FB1: %08X",
  _leftFramebuffers[1]).c_str(), 10, posY, Color::Blank,
  Color::Black); Renderer::DrawString(Utils::Format("Sel: %d",
  _originalBuffer).c_str(), 10, posY, Color::Blank, Color::Black);
  }
  else
  {
...
u32 ScreenImpl::AcquireFromGsp(bool fade)
{
  ...
  while ((GPU_PSC0_CNT | GPU_PSC1_CNT | GPU_TRANSFER_CNT |
          GPU_CMDLIST_CNT) &
         1)
    ;
...
{
  if (!SystemImpl::IsCitra)
    svcFlushDataCacheRange(GetLeftFrameBuffer(),
                           GetFrameBufferSize());
...
void ScreenImpl::GetFrameBufferInfos(
  int &rowStride, int &bpp, GSPGPU_FramebufferFormat &format) const
{
  rowStride = _stride;
  bpp = _bytesPerPixel;
  format = _format;
...
u8 *ScreenImpl::GetLeftFrameBuffer(int posX, int posY)
{
  ...
  u32 offset =
    (_rowSize - 1 - posY + posX * _rowSize) * _bytesPerPixel;

  return reinterpret_cast<u8 *>(_leftFrameBuffers[!_currentBuffer]) +
         offset;
}
...
u8 *ScreenImpl::GetRightFrameBuffer(int posX, int posY)
{
  ...
  return reinterpret_cast<u8 *>(_rightFrameBuffers[!_currentBuffer]) +
         offset;
}

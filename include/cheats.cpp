...
u32 GetFNsPointer()
{
  if (IsInRace() && Process::Read32(0xFFFF6F0, offset) &&
...
{
  if (IsInRace() && Process::Read32(0x65C528, offset) &&
      is_in_range(offset, 0x14000000, 0x18000000)) {
...
void stalking(MenuEntry *entry)
{
  ...
  if (Controller::IsKeysPressed(Y + DPadRight) &&
      player < (GetTotalPlayers() ? GetTotalPlayers() + 1 : 9))
    player++;
...
void aimbot(MenuEntry *entry)
{
  ...
  if (!in_race) {
    ...
    if ((GetTotalPlayers() != 0 && playerID > GetTotalPlayers()) ||
...
void cpuBrawl(MenuEntry *entry)
{
  ...
  for (int i = 2; i < 9; i++) {
    ...
    if (is_in_range(offset, 0x14000000, 0x18000000) &&
        is_in_range(pointer2, 0x14000000, 0x18000000))
...
void TouchCode(MenuEntry *entry)
{
  ...
  if (Process::Read32(0x6789C8, data) && Process::Read32(data + 0x1A48, data) && data == 1 &&
    ...
    if (Process::Read32(0x140002F4, map_pointer) &&
        Process::Read32(map_pointer + 0x14, map_pointer) &&
        Process::Read32(map_pointer + 0x370, map_pointer) &&
        Process::Read32(map_pointer + 0x270, map_pointer) &&
        Process::Read32(map_pointer + 0x60, map_pointer) &&
        Process::Read32(map_pointer + 0x688, map_pointer)) {
...
void eliminationMode(MenuEntry *entry)  // credit to fish for the original asm code
{
  ...
  address = 0x5EBAF8;
  static const u8 buffer2[] = {0x6C, 0x60, 0xA0, 0xE3, 0x59, 0x60,
                               0xC4, 0xE5, 0xE,  0xF0, 0xA0, 0xE1};
